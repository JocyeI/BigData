...
![ArrayDeque_base](Images/ArrayDeque_base.png)



上图中看到，**`head`指向首端第一个有效元素，`tail`指向尾端第一个可以插入元素的空位**。因为是循环数组，所以`head`不一定总等于0，`tail`也不一定总是比`head`大。
...
![ArrayDeque_addFirst](Images/ArrayDeque_addFirst.png)
...
![ArrayDeque_doubleCapacity](Images/ArrayDeque_doubleCapacity.png)
...
![ArrayDeque_addLast](Images/ArrayDeque_addLast.png)
