...
![java-thread-x-juc-aqs-1](Images/java-thread-x-juc-aqs-1.png)
...
![java-thread-x-juc-aqs-2](Images/java-thread-x-juc-aqs-2.png)
...
![java-thread-x-juc-aqs-3](Images/java-thread-x-juc-aqs-3.png)
...
![java-thread-x-juc-aqs-4](Images/java-thread-x-juc-aqs-4.png)
...
![java-thread-x-juc-aqs-5](Images/java-thread-x-juc-aqs-5.png)
...
![java-thread-x-juc-aqs-6](Images/java-thread-x-juc-aqs-6.png)

说明: 经过一系列的方法调用，最后达到的状态是禁用t2线程，因为调用了LockSupport.lock。

- t1线程调用lock.unlock，其方法调用顺序如下，只给出了主要的方法调用。

![java-thread-x-juc-aqs-7](Images/java-thread-x-juc-aqs-7.png)
...
![java-thread-x-juc-aqs-8](Images/java-thread-x-juc-aqs-8.png)
...
![java-thread-x-juc-aqs-9](Images/java-thread-x-juc-aqs-9.png)
...
![java-thread-x-juc-aqs-10](Images/java-thread-x-juc-aqs-10.png)
...
![java-thread-x-juc-aqs-15](Images/java-thread-x-juc-aqs-15.png)

说明: 此时，p2线程所在的结点为头结点，并且其他两个线程(c1、c2)依旧被禁止，所以，此时p2线程继续运行，执行用户逻辑。

- p2线程执行fullCondition.await，其方法调用顺序如下，只给出了主要的方法调用。

![java-thread-x-juc-aqs-17-1](Images/java-thread-x-juc-aqs-17-1.png)
...
![java-thread-x-juc-aqs-16](Images/java-thread-x-juc-aqs-16.png)

说明: 其中，head设置为包含c1线程的结点，c1继续运行。

- c1线程执行fullCondtion.signal，其方法调用顺序如下，只给出了主要的方法调用。

![java-thread-x-juc-aqs-17](Images/java-thread-x-juc-aqs-17.png)
...
![java-thread-x-juc-aqs-18](Images/java-thread-x-juc-aqs-18.png)

说明: 最终c2线程会获取锁资源，继续运行用户逻辑。

- c2线程执行emptyCondition.await，由前面的第七步分析，可知最终的状态如下。

![java-thread-x-juc-aqs-19](Images/java-thread-x-juc-aqs-19.png)
...
![java-thread-x-juc-aqs-20](Images/java-thread-x-juc-aqs-20.png)

- p2继续运行，执行emptyCondition.signal，根据第九步分析可知，最终到达的状态如下。

![java-thread-x-juc-aqs-21](Images/java-thread-x-juc-aqs-21.png)
...
![java-thread-x-juc-aqs-22](Images/java-thread-x-juc-aqs-22.png)
